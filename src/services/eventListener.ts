...
// Helper: Convert byte array to hex string with 0x prefix
...
export class OnChainEventListener {
  ...
  async start() {
    ...
    console.log(`[EventListener] Starting on-chain event monitoring for program: ${this.programId.toBase58()}`);
    ...
    try {
      await prisma.tx.upsert({
        ...
        update: {
          ...
          merkle_root: toHex(event.newMerkleRoot),
          signature: txSignature,
  ...
  private async storeTransferEvent(event: any, txSignature: string) {
    try {
      ...
      await prisma.tx.upsert({
        ...
        create: {
          chain: "solana",
          commitment: toHex(event.out1Commitment),
          leaf_index: event.nextLeafIndex,
          merkle_root: toHex(event.newMerkleRoot1),
          signature: txSignature,
          event: "TransferCompleted",
      ...
      // Store out2
      await prisma.tx.upsert({
        where: { commitment: toHex(event.out2Commitment) },
        update: {
        ...
        create: {
          chain: "solana",
          commitment: toHex(event.out2Commitment),
          leaf_index: event.nextLeafIndex + 1,
          merkle_root: toHex(event.newMerkleRoot2),
          signature: txSignature,
          event: "TransferCompleted",
